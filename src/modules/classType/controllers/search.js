...
const classTypeSearch = async (req, res) => {
  ...
  try {
    ...
    const totalCountPromise = ClassType.countDocuments(query); // Находим кол-во результатов
    const searchPromise = classTypeSearchQuery({ query, page, limit }); // Находим результат
    ...
    res.status(200).json(message.success('ClassTypeSearch ok', result));
  } catch (error) {
    const analyticsId = analytics('ClassType_SEARCH_ERROR', {
      error,
      body: req.body,
      entity: 'ClassType',
      user: userId,
      controller: 'classTypeSearch',
    });

    res.status(400).json(message.fail('ClassTypeSearch error', analyticsId));
  }
};

module.exports = classTypeSearch;

function classTypeSearchQuery({ query, page, limit }) {
  return ClassType.find(query)
    .sort({ name: 1 })
    .limit(limit)
    .skip(limit * (page - 1))
    .exec()
    .then((docs) => {
      return message.success('ClassType found', docs);
    });
}
